...
![alt text](/images/part12/2025-12-09-spring8/image.png)
...
## 구글 클라우드 프로젝트 생성하기
![alt text](/images/part12/2025-12-09-spring8/image1.png)
...
![alt text](/images/part12/2025-12-09-spring8/image2.png)
- API 및 서비스 -> 사용자 인증 정보 -> Oauth동의 화면 -> 시작 -> 앱정보작성 및 만들기
- 외부 -> 이메일 -> 만들기
<br/><br/>

![alt text](/images/part12/2025-12-09-spring8/image3.png)
![alt text](/images/part12/2025-12-09-spring8/image4.png)
```bash
...
sequenceDiagram
    ...
    A ->> C: POST /auth/google/mobile-login<br/>GoogleAppLoginRequestDto(idToken)
    C ->> S: mobileLogin(GoogleAppLoginRequestDto)

    S ->> S: parseIdTokenToProfile(idToken)
    S ->> V: verify(idToken)
    V -->> S: GoogleIdToken

    S ->> S: payload 추출<br/>(sub, email, name, picture)
    S -->> S: GoogleUserInfoResponseDto

    S ->> R: findByUid(sub)
    R ->> D: SELECT user WHERE uid=sub
    D -->> R: User or null

    alt User exists
        R -->> S: User(Entity)
    else User not exists
        S ->> S: User.builder()<br/>uid, email, provider, role
        S ->> R: save(User)
        R ->> D: INSERT user
        D -->> R: saved User
        R -->> S: User(Entity)
    ...
    S ->> S: LoginResponseDto.from(User)
...
```java
/**
 GoogleAppLoginRequestDto
 로그인 버튼 클릭 직후에 사용됩니다.
 모바일에서 받은 google idToken을 서버의 AuthController로 보낼 때 사용합니다.
 */
@Getter
@NoArgsConstructor
...
/**
 GoogleUserInfoResponseDto
 ViewController -> Service -> mobileLogin -> parsedIdTokenToProfile()의 성공 직후
 Google이 서명한 토큰에서 정보를 추출한 뒤 
 Google이 보증한 사용자 정보 묶음을 반환합니다.
 */
@Getter
...
/**
 LoginResponseDto
 mobileLogin()에서 최종 반환값으로 현재 유저 상태를 클라이언트에게 알려줍니다.
 클라이언트에서 홈 / 닉네임 분기의 기준점
 */
@Getter
@NoArgsConstructor
...
/**
 SignupRequestDto
 로그인은 성공했지만 회원가입을 아직 하지 않은 클라이언트가 값을 채우고 최종 회원가입을 요청합니다.
 */
@Getter
...
/**
완전히 가입 된 유저 데이터를 반환합니다. 
클라이언트는 이 응답을 보고 홈 화면으로 이동합니다.
*/
@Getter
@NoArgsConstructor
